...
void Visualizer::initScene(int assignment) {
    ...
    {
		//Camera
		mp_Ren->GetActiveCamera()->SetPosition(0.05,0.5,0.55);
...
void Visualizer::drawTDCR(int disks_per_seg, std::array<double,1> pradius_disks_, double radius_disks, double ro, double height_disks)
{
	//Remove old robot
	removeTDCR();
	
    std::array<double, 3> pradius_disks = {pradius_disks[0],0, 0};
...
void Visualizer::updateCTCR(Eigen::MatrixXd backbone_centerline, std::vector<int> tube_ind)
{
    ...
    for(int k = 0; k < 6*m_ctcrVis.pts_per_seg+1; k++)
    {
            if(k != 0 && k < backbone_centerline.cols()/4)
            {
            ...
            else if(k >= backbone_centerline.cols()/4)
            {
					m_ctcrVis.tube_lines.at(0).at(k-1)->SetPoint1(0,0,0);
					m_ctcrVis.tube_lines.at(0).at(k-1)->SetPoint2(0,0,0);
					m_ctcrVis.tube_lines.at(1).at(k-1)->SetPoint1(0,0,0);
...
void Visualizer::drawTargetFrame(Eigen::Matrix4d frame)
{
	
	//VTK Actor for target frame
    mp_target_frame = vtkSmartPointer<vtkAxesActor>::New();
	mp_target_frame->SetXAxisLabelText("");
	mp_target_frame->SetYAxisLabelText("");
	mp_target_frame->SetZAxisLabelText("");
	mp_target_frame->SetShaftTypeToCylinder();
	mp_target_frame->SetCylinderRadius(0.05);
	mp_target_frame->SetTotalLength(0.02,0.02,0.02);
	mp_target_frame->GetXAxisTipProperty()->SetColor(0.9,0.9,0.9);
	mp_target_frame->GetYAxisTipProperty()->SetColor(0.9,0.9,0.9);
	mp_target_frame->GetZAxisTipProperty()->SetColor(0.9,0.9,0.9);
	mp_target_frame->GetXAxisShaftProperty()->SetColor(0.9,0.9,0.9);
	mp_target_frame->GetYAxisShaftProperty()->SetColor(0.9,0.9,0.9);
	mp_target_frame->GetZAxisShaftProperty()->SetColor(0.9,0.9,0.9);
    mp_Ren->AddActor(mp_target_frame);
    ...
    for(int i = 0; i < 4; i++)
    {
        for(int j = 0; j < 4; j++)
        {
            ee_frame_vtk->SetElement(i,j,frame(i,j));
        }
    }
    mp_target_frame->SetUserMatrix(ee_frame_vtk);
}

void Visualizer::moveTargetFrame(Eigen::Matrix4d frame)
{
    vtkSmartPointer<vtkMatrix4x4> ee_frame_vtk = vtkSmartPointer<vtkMatrix4x4>::New();
    for(int i = 0; i < 4; i++)
    {
...
void Visualizer::removeTargetFrame()
{
	mp_Ren->RemoveActor(mp_target_frame);
	mp_target_frame = NULL;
}

void Visualizer::drawPath(std::vector<Eigen::Matrix4d> path)
{
	//Draw line segments that connect the discrete points handed in the curve data structure
	...
	for(int i = 1; i < path.size(); i++)
	{
		...
		vtkSmartPointer<vtkActor> curveActor = vtkSmartPointer<vtkActor>::New();
		curveActor->SetMapper(curveMapper);
		curveActor->GetProperty()->SetColor(1,0,0);
		curveActor->GetProperty()->SetAmbient(0.4);
		curveActor->GetProperty()->SetDiffuse(0.6);
		curveActor->GetProperty()->SetSpecular(0.0);
		mp_Ren->AddActor(curveActor);
